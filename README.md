...
Comparing, all of the embedding visualizations, the single-layer ResNet50 embeddings stand out simply for the fact that there is only a single plot of embeddings as embeddings were only extracted from one layer in this method. Comparing the shape and distribution of the embeddings, they appear visually similar to the species-level embeddings from the ResNet50 and ResNet101 double layer extraction methods. This makes sense as both extractions occured at the final global average pool layer of the ResNet model architecture. 

The double layer feature extraction using ResNet50 and ResNet101 produce embeddings that are quite similar. It appears that the ResNet101 double layer-extracted embeddings are sort of an inverted (across the middle of the plot) version of the embedding plots from the same feature extraction method using a ResNet50 model. The species-level embeddings of both ResNet50 and ResNet101 double-layer feature extraction methods are strikingly similar - simply inverted/transformed. However, upon further inspection, there are some differences in the embeddings, particularly the when comparing the genus-level embeddings of both extraction methods. I would say that the ResNet101 species-level embeddings are slightly more separated than the ResNet50 embeddings. The genus-level embeddings of the ResNet101 method seem slightly less separated than the genus-level embeddings of the ResNet50 method. As mentioned previously, when using the same clustering approach in the "Clustering-BioTrove" challenge, the ResNet101 double-layer extracted model performed slightly better than the ResNet50 version which would suggest that the separability of extracted species-level embeddings had an influence on clustering performance.
